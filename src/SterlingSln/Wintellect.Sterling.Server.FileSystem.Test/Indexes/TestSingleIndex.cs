...
namespace Wintellect.Sterling.Server.FileSystem.Test.Indexes
{
    [TestClass]
    public class TestSingleIndex
    {
        ...
        public void Init()
        {
            if (_driver == null)
            {
                _driver = new FileSystemDriver(_testDatabase.Name, new DefaultSerializer(), SterlingFactory.GetLogger().Log);
            }
        ...
        public void TestSerialization()
        {
            ...
            var file = new FileSystemHelper();

            file.EnsureDirectory(pathProvider.GetDatabasePath("Databases/", _testDatabase.Name, _driver));
            file.EnsureDirectory(pathProvider.GetTablePath("Databases/", _testDatabase.Name, typeof(TestModel), _driver));
