...
namespace Wintellect.Sterling.IsolatedStorage.Test.Indexes
{
    [Tag("Index")]
    [TestClass]
    public class TestSingleIndex
    {
        private IsolatedStorageDriver _driver;
        ...
        public void Init()
        {
            if (_driver == null)
            {
                _driver = new IsolatedStorageDriver(_testDatabase.Name, new DefaultSerializer(), SterlingFactory.GetLogger().Log);
            }
        ...
        {
            // need to use the iso helper and path provides to ensure some directories
            var pathProvider = new PathProvider();
            var iso = new IsoStorageHelper(false);

            iso.EnsureDirectory(pathProvider.GetDatabasePath("Sterling/", _testDatabase.Name, _driver));
            iso.EnsureDirectory(pathProvider.GetTablePath("Sterling/", _testDatabase.Name, typeof(TestModel), _driver));
